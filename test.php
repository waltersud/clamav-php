...
$clamav = new Clamav();
if(file_put_contents("$test_dir/clamav_test_good.txt", $good_test)) {
	echo "Testing a good file...\n";
	echo $test_dir;

    if($clamav->scan("$test_dir/clamav_test_good.txt")) {
...
if(file_put_contents("$test_dir/clamav_test_bad.txt", $bad_test)) {
    echo "Testing a bad file...\n";
    if($clamav->scan("$test_dir/clamav_test_bad.txt")) {
        echo "YAY, file is safe!\n";
    } else {
        echo "BOO, file is a virus.  Message: " . $clamav->getMessage() . "\n";
    }
